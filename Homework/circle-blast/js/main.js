...
function createLabelsAndButtons(){
    ...
    playAgainButton.on('pointerover',e=>e.target.alpha = 0.7); // concise arrow function
...
function gameLoop(){
    ...
    let amt = 6 * dt; // at 60 FPD would move about 10% of distance per update
...
function end() {
    paused = true;
    // clear out level
    circles.forEach(c=>gameScene.removeChild(c)); // consice arrow function with no brackets and no return
    circles = [];
...
function createExplosion(x,y,frameWidth,frameHeight) {
    // http://pixijs.download/dev/docs/PIXI.extras.AnimateSprite.html
    // the animation framers are 64x64 pixels
    let w2 = frameWidth/2;
